...
package {
	...
	public class cdsfmsplayer extends Sprite {
		...
		private var PlayList:Array;// Include object array
		private var PlayIndex:int = 0;
		private var LoopBool:Boolean = false;//no loop default
		
		public function cdsfmsplayer() {
			...
			ExternalInterface.addCallback("sendListToFlash", getListFromJavaScript);
			var myvid:Video = new Video(320, 240);
			myvid.x = 0;
			myvid.y = 80;
			c.clientvideo=myvid;
			addChild(myvid);
			//var testArray:Array = ["rtmp://172.22.28.66/vod/sample","rtmp://10.74.61.77/vod/sample"];
			//getListFromJavaScript(testArray,"true");
		}
		...
		public function playNext():void {
			if (!LoopBool && PlayIndex == PlayList.length ) return;
			if (PlayIndex == PlayList.length && LoopBool) PlayIndex = 0;
			var currentIndex:int = PlayIndex;
			var myobj:Object = PlayList[currentIndex];
			startPlay(myobj.NetCON, myobj.Stream);
			PlayIndex ++;
		}
		public function startPlay(conn:String, stream:String): void {
			trace("startplay " + conn + " Stream " + stream);
			var mystreamname:String = stream;
			var myurl:String = conn;
...
/*			StreamList = new Array();
		...
		public function getListFromJavaScript(StreamListJS:Array, isLoop:String):void {
			debugtest.appendText("ENTER");
			if (isLoop == "true" || isLoop == "TRUE" ) {
				LoopBool = true;
			}

			for (var i:int = 0; i < StreamListJS.length; i++) {
				var url:String = StreamListJS[i];
				var url_stream:Array = parseStreamFromURL(url);
				var streamType:String = "VOD";
				var myobj:Object = { NetCON:url_stream[0], Stream:url_stream[1], Type:"VOD" };
				PlayList.push(myobj);
				switch(streamType) {
			...
			debugtest.appendText(" PlayList Length = " + StreamListJS.length);
		...
		public function parseStreamFromURL(str:String):Array {
			//Using the new parser,NetConnect Syntax:
			//protocol:[//host][:port]/appname[/instanceName]
			//So only the stream name required, the other part should be Connect Parameter.
			//
